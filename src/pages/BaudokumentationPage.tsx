...
export default function BaudokumentationPage() {
  ...
  const [records, setRecords] = useState<Baudokumentation[]>([]);
  const [loading, setLoading] = useState(true);
  const [search, setSearch] = useState('');
  const [dialogOpen, setDialogOpen] = useState(false);
  const [editingRecord, setEditingRecord] = useState<Baudokumentation | null>(null);
  const [deleteTarget, setDeleteTarget] = useState<Baudokumentation | null>(null);
  ...
  async function handleCreate(fields: Baudokumentation['fields']) {
  ...
  async function handleUpdate(fields: Baudokumentation['fields']) {
    ...
    await LivingAppsService.updateBaudokumentationEntry(editingRecord.record_id, fields);
    await loadData();
  ...
  async function handleDelete() {
    ...
    await LivingAppsService.deleteBaudokumentationEntry(deleteTarget.record_id);
  ...
  return (
    <PageShell
      ...
      subtitle={`${records.length} ${t('in_system', { entity: appLabel('baudokumentation') })}`}
      action={
    ...
    >
      <div className="relative w-full max-w-sm">
        ...
        <Input
          placeholder={t('search_entity', { entity: appLabel('baudokumentation') })}
          value={search}
        ...
        <Table className="[&_tbody_td]:px-6 [&_tbody_td]:py-2 [&_tbody_td]:text-base [&_tbody_td]:font-medium [&_tbody_tr:first-child_td]:pt-6 [&_tbody_tr:last-child_td]:pb-10">
            ...
            <TableRow className="border-b border-input">
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('auftrag')}>
                <span className="inline-flex items-center gap-1">
                  {fieldLabel('baudokumentation', 'auftrag')}
              ...
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('foto')}>
                <span className="inline-flex items-center gap-1">
                  {fieldLabel('baudokumentation', 'foto')}
              ...
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('notiz')}>
                <span className="inline-flex items-center gap-1">
                  {fieldLabel('baudokumentation', 'notiz')}
              ...
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('aufnahmedatum')}>
                <span className="inline-flex items-center gap-1">
                  {fieldLabel('baudokumentation', 'aufnahmedatum')}
          ...
          <TableBody>
            {sortRecords(filtered).map(record => (
              <TableRow key={record.record_id} className="hover:bg-muted/50 transition-colors cursor-pointer" onClick={(e) => { if ((e.target as HTMLElement).closest('button, [role="checkbox"]')) return; navigate(`/baudokumentation/${record.record_id}`); }}>
            ...
            {filtered.length === 0 && (
              <TableRow>
                <TableCell colSpan={5} className="text-center py-16 text-muted-foreground">
                  {search ? t('no_results') : t('no_data_yet', { entity: appLabel('baudokumentation') })}
                </TableCell>
              </TableRow>
            )}
          </TableBody>
