...
import type { Kunden, Monteure, Auftraege, Baudokumentation, CreateKunden, CreateMonteure, CreateAuftraege, CreateBaudokumentation } from '@/types/app';
...
export class LivingAppsService {
  ...
  static async getMonteure(): Promise<Monteure[]> {
    ...
    return hydrateRecords(records, 'monteure');
  }
  static async getMonteureEntry(id: string): Promise<Monteure | undefined> {
    ...
    const record = { record_id: data.id, ...data, createdat: data.created_at ?? '', updatedat: data.updated_at ?? null } as Monteure;
    return hydrateRecords([record], 'monteure')[0];
  }
  static async createMonteureEntry(fields: CreateMonteure): Promise<MutationResult> {
    const data = await callApi('POST', `/apps/${APP_IDS.MONTEURE}/records`, { fields: cleanFieldsForApi(fields as any, 'monteure') });
    return { ...data, record_id: data.id };
  }
  static async updateMonteureEntry(id: string, fields: Partial<CreateMonteure>): Promise<MutationResult> {
    const data = await callApi('PATCH', `/apps/${APP_IDS.MONTEURE}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'monteure') });
    return { ...data, record_id: data.id };
  }
  ...
  // --- BAUDOKUMENTATION ---
  static async getBaudokumentation(): Promise<Baudokumentation[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.BAUDOKUMENTATION}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec,
      createdat: rec.created_at ?? '', updatedat: rec.updated_at ?? null,
    })) as Baudokumentation[];
    return hydrateRecords(records, 'baudokumentation');
  }
  static async getBaudokumentationEntry(id: string): Promise<Baudokumentation | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.BAUDOKUMENTATION}/records/${id}`);
    const record = { record_id: data.id, ...data, createdat: data.created_at ?? '', updatedat: data.updated_at ?? null } as Baudokumentation;
    return hydrateRecords([record], 'baudokumentation')[0];
  }
  static async createBaudokumentationEntry(fields: CreateBaudokumentation): Promise<MutationResult> {
    const data = await callApi('POST', `/apps/${APP_IDS.BAUDOKUMENTATION}/records`, { fields: cleanFieldsForApi(fields as any, 'baudokumentation') });
    return { ...data, record_id: data.id };
  }
  static async updateBaudokumentationEntry(id: string, fields: Partial<CreateBaudokumentation>): Promise<MutationResult> {
    const data = await callApi('PATCH', `/apps/${APP_IDS.BAUDOKUMENTATION}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'baudokumentation') });
    return { ...data, record_id: data.id };
  }
  static async deleteBaudokumentationEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.BAUDOKUMENTATION}/records/${id}`);
  }
